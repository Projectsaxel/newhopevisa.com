#!/usr/bin/env python3
"""Injeta popup USCIS inline em todas as páginas /es/."""
...
JS_FILE = ROOT / "es-uscis-popup.js"
STYLE_MARKER = "<!-- site-uscis-popup-es-style -->"
SCRIPT_MARKER = "<!-- site-uscis-popup-es -->"
...
OLD_STYLE_RE = re.compile(
	r"\n<!-- site-uscis-popup-es-style -->.*?</style>\n",
	re.DOTALL,
)
...
def iter_es_pages() -> list[Path]:
	paths: list[Path] = []
	home = ES / "index.html"
	if home.is_file():
		paths.append(home)
	for child in sorted(ES.iterdir()):
